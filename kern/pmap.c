...
boot_alloc(uint32_t n)
{
	...
	static size_t ppages_allocated;
	size_t ppages_required;
	char *result;
	...
	if (n > 0) {
		ppages_required = n / PGSIZE + (n % PGSIZE == 0 ? 0 : 1);
		if (ppages_allocated + ppages_required > npages) {
			panic("boot_alloc: not enough physical memory\n");
		}
		ppages_allocated += ppages_required;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (start_addr = (uintptr_t)va; start_addr < (uintptr_t)va + len; start_addr += PGSIZE) {
		if (start_addr > ULIM) {
			user_mem_check_addr = start_addr;
			return -E_FAULT;
		}

		pte = pgdir_walk(env->env_pgdir, (void *)start_addr, 0);
